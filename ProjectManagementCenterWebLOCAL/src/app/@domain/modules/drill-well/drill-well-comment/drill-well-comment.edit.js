...
export const DrillWellCommentEdit = props => {
  ...
  const submitRemote = obj => {
    ...
    spinner.show();
    const remoteObj = drillWellCommentModel.toRemoteObj(obj);
    api.drillWellCommentApi.update(remoteObj).then(() => {
      submitLocal(obj);
  ...
  const submitLocal = obj => {
    ...
    obj = drillWellCommentModel.toLocalObj(obj);
    const objs = [...comments()];
